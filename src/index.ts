...
// Callback interface
function topologicalScheduler<T, R>(graph: DependencyGraph<T>, worker: WorkerFunction<T, R>, options: SchedulerOptions, callback: SchedulerCallback<T, R>): undefined;
...
// Implementation
function topologicalScheduler<T, R>(graph: DependencyGraph<T>, worker: WorkerFunction<T, R>, options?: SchedulerOptions | SchedulerCallback<T, R>, callback?: SchedulerCallback<T, R>): undefined | Promise<SchedulerResult<T, R>[]> {
  // Normalize arguments
  ...
  if (typeof callback === 'function') {
    schedule(graph, worker, options, callback);
    return undefined;
  }
